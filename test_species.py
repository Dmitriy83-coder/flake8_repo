...
CONNECTION_STRING = "postgresql://postgres:12345@127.0.0.1:5432/QA-Testing"
...
@pytest.fixture
def test_species(db):
    """создаём тестовую запись и удаляем после теста"""
    test_name = "насекомые"
...
def test_add_species(db):
    """тестим добавление новой сущности"""
    ...
    # проверяем, что вид добавился с правильными данными
...
def test_update_species(db, test_species):
    """тестим изменения уже имеющейся сущности"""
    # подготовка тестовых данных
    ...
    # возвращаем исходное название для финальной очистки
...
def test_delete_species(db):
    ...
    # проверяем успешность, что мы выполнили
    assert result is True
...
def test_add_duplicate_species(db):
    """тестим добавление дубликата (негативный тест)"""
    test_name = "млекопитающие"
    
    # убедимся, что такого вида нет
    existing = db.get_species_by_name(test_name)
    if existing:
        db.delete_species_by_name(test_name)
    ...
    # пытаемся добавить второй раз (должна быть ошибка или вернуть None)
    try:
        second_id = db.add_species(test_name)
        # если в БД появится дубликат - удаляем второй
        if second_id:
            db.delete_species(second_id)
    except Exception as e:
        # ожидаемая ошибка при дублировании
